...
std::optional<Mesh> load_mesh(pxr::UsdPrim &prim, MeshContext &context) {
    ...
    auto normal_matrix = transform.GetOrthonormalized().ExtractRotationMatrix();


    return Mesh{.normal_matrix = mat3(pxr_to_vec3(normal_matrix.GetRow(0)), pxr_to_vec3(normal_matrix.GetRow(1)), pxr_to_vec3(normal_matrix.GetRow(2))),
                .scene = mesh_scene,
