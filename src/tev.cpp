...
struct TevConnection {
    ...
    void send_create(const std::string& image_name, uint32_t width, uint32_t height, const std::vector<std::string>& channel_names) {
    ...
    void send_update(
        ...
        const std::vector<uint64_t>& channel_offsets,
        const std::vector<uint64_t>& channel_strides,
        ...
        uint32_t length = 4 + (1 * 2) + image_name.length() + 1 + (4 * 5) + channel_offsets.size() * 8 + channel_strides.size() * 8 + data.size() * 4;
        for (auto &channel_name: channel_names) {
            length += channel_name.length() + 1;
        }
